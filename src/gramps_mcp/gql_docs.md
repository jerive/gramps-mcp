...
> Reference copied for use as an MCP resource (`gramps://gql-spec`) so an
> agent can look up GQL syntax before writing a `gql=` filter against a
> mounted tree. GQL is a feature of
> [Gramps Web API](https://github.com/gramps-project/gramps-web-api)
> itself, not something this project defines.

GQL, a query language to a [Gramps](https://github.com/gramps-project/gramps) database. The syntax is inspired by JQL, the Jira Query Language.
